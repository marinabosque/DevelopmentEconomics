 ...
 ***********************************************************************************
	 ...
	 ** Histogram of betas and phis
	 preserve
		 ...
		 ** Mean and median
		 ...
		 histogram beta, title("Figure 3: Coefficient beta across households", color(black)) ///
		 xtitle ("Beta") graphregion(color(white)) bcolor(maroon)
		 graph export "${output}hist_beta_urban.png", replace
	 restore
	 ...
	 preserve
		 ...
		 ** Mean and median
		 ...
		 histogram phi, title("Figure 4: Coefficient phi across households", color(black)) ///
		 xtitle ("Phi") graphregion(color(white)) bcolor(navy)
		 graph export "${output}hist_phi_urban.png", replace
	 restore
	 ...
	 ** Define five income groups
	 sort income_i
	 gen nobs = _N // total of 609 observations
	 gen nhh = _n 
	 
	 gen inc_group = 0
	 replace inc_group = 1 if nhh<=122
	 replace inc_group = 2 if nhh>122 & nhh<=244
	 replace inc_group = 3 if nhh>244 & nhh<=365
	 replace inc_group = 4 if nhh>365 & nhh<=487
	 replace inc_group = 5 if nhh>487 & nhh<=609
	 ...
	 ** Define five beta groups
	 ...
	 gen beta_group = 0
	 replace beta_group = 1 if nhh<=122
	 replace beta_group = 2 if nhh>122 & nhh<=244
	 replace beta_group = 3 if nhh>244 & nhh<=365
	 replace beta_group = 4 if nhh>365 & nhh<=487
	 replace beta_group = 5 if nhh>487 & nhh<=609
